...
function timestamp(row) {
  return row.examined_at || row.created_at || null
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const days = Array.from({ length: 7 }, (_, index) => {
      ...
      return { key: dateKey(date), label: weekday(date), count: 0 }
    })
    const dayByKey = new Map(days.map((day) => [day.key, day]))
    for (const exam of confirmed) {
      const key = dateKey(timestamp(exam))
    ...
    for (const user of users.filter((item) => item.account_status === 'active')) {
      ...
      if (severe.length) {
        followups.push({
          ...
          issue: `${[...new Set(severe.map((finding) => finding.disease_name_snapshot))].join(', ')} · รุนแรง`,
          time: thaiDate(lastAt),
          severe: true,
        })
      } else if (!latest || (daysSince != null && daysSince >= 7)) {
        followups.push({
          userId: user.user_id,
          username: user.username,
          name: user.display_name || user.username,
          issue: latest ? `ไม่ได้ตรวจ ${daysSince} วัน` : 'ยังไม่เคยตรวจ',
          time: thaiDate(lastAt),
          severe: false,
        })
      }
    ...
    return sendJson(res, 200, {
      ...
      severeCount,
      completedLast7Days,
      averagePerDay: Number((completedLast7Days / 7).toFixed(1)),
      activityLast7Days: days,
    ...
    return sendJson(res, 500, { message: 'ไม่สามารถโหลดข้อมูลภาพรวมได้' })
  }
}
