...
function HumanReview({ photos, diseaseRecords, aiFindings, confirmedFindings, setConfirmedFindings, submitError, onSubmit, isSubmitting, onBack }: { photos: Partial<Record<FootPosition, string>>; diseaseRecords: Disease[]; aiFindings: Finding[]; confirmedFindings: Finding[]; setConfirmedFindings: React.Dispatch<React.SetStateAction<Finding[]>>; submitError?: string; onSubmit: () => void; isSubmitting?: boolean; onBack: () => void }) {
  ...
  return (
    <div className="page narrow-page human-review-page">
      ...
      <div className="review-explainer"><Info size={19} /><p>ผล AI ต้นฉบับจะถูกเก็บแยกจากรายการที่คุณยืนยัน เพื่อให้แพทย์ตรวจสอบย้อนหลังได้</p></div>
      {submitError ? <div className="form-error review-submit-error" role="alert"><AlertTriangle size={18} />{submitError}</div> : null}
      <button className="button button-primary button-large" type="button" disabled={isSubmitting} onClick={onSubmit}><FileCheck2 size={21} />{isSubmitting ? 'กำลังเตรียมภาพสรุป…' : 'ยืนยันและส่งผลตรวจ'}</button>
...
function SummaryReport({ examination, photos, diseaseRecords, onHome, onRestart }: { examination: Examination; photos: Partial<Record<FootPosition, string>>; diseaseRecords: Disease[]; onHome: () => void; onRestart: () => void }) {
  ...
  return (
    <div className="page narrow-page summary-page">
      ...
      {comparisonFinding ? <section className="comparison-card"><div className="comparison-icon"><TrendingDown size={24} /></div><div><span className="eyebrow">เทียบกับครั้งก่อน</span><h2>{comparisonFinding.name} {comparisonFinding.comparison}</h2><p>ผลเปรียบเทียบจากการตรวจครั้งก่อน ควรติดตามตามคำแนะนำของแพทย์</p></div></section> : null}
      <section className="recommendation-card"><div className="section-heading"><div><span className="eyebrow">คำแนะนำจากแพทย์</span><h2>ดูแลต่อเนื่องหลังการตรวจ</h2></div><HeartPulse size={24} /></div><ul>{(recommendationLines.length ? recommendationLines : fallbackRecommendations).map((recommendation) => <li key={recommendation}><Check size={17} />{recommendation}</li>)}</ul></section>
      <ClinicalDisclaimer />
...
function HistoryCalendar({ examinations: patientExaminations, onSelect }: { examinations: Examination[]; onSelect: (exam: Examination) => void }) {
  ...
  const selectedExam = patientExaminations.find((exam) => exam.date === selectedDate)
  ...
  return (
    <div className="calendar-layout"><section className="calendar-card"><div className="calendar-header"><button className="icon-button" type="button" aria-label="เดือนก่อน" onClick={() => shiftMonth(-1)}><ArrowLeft size={19} /></button><h2>{monthNames[cursor.month]} {cursor.year + 543}</h2><button className="icon-button" type="button" aria-label="เดือนถัดไป" onClick={() => shiftMonth(1)}><ArrowRight size={19} /></button></div><div className="weekdays">{['อา','จ','อ','พ','พฤ','ศ','ส'].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{Array.from({ length: firstDay }).map((_, index) => <span key={`empty-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => index + 1).map((day) => { const date = `${monthPrefix}-${String(day).padStart(2, '0')}`; const exam = patientExaminations.find((item) => item.date === date); return <button key={day} type="button" aria-label={`เลือกวันที่ ${day} ${monthNames[cursor.month]} ${cursor.year + 543}`} className={`${exam ? 'has-exam ' : ''}${selectedDay === day ? 'selected' : ''}`} onClick={() => { setSelectedDay(day); if (exam) onSelect(exam) }}>{day}{exam ? <i /> : null}</button> })}</div></section><section className="calendar-side"><span className="eyebrow">วันที่เลือก</span><h2>{selectedDay} {monthNames[cursor.month]} {cursor.year + 543}</h2>{selectedExam ? <div className="calendar-event"><span><Check size={16} /></span><div><strong>ตรวจเท้า เวลา {selectedExam.time} น.</strong><p>พบ {selectedExam.findings.map((finding) => finding.name).join(' และ ')}</p><button type="button" onClick={() => onSelect(selectedExam)}>ดูผลตรวจ <ChevronRight size={16} /></button></div></div> : <div className="calendar-empty"><CalendarDays size={22} /><p>ไม่มีรายการตรวจในวันนี้</p></div>}</section></div>
  )
}
...
function DiseaseFormModal({ disease, onClose, onSave }: { disease: Disease | null; onClose: () => void; onSave: (draft: Omit<Disease, 'id'>) => void | Promise<void> }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    onSave({ ...draft, name: draft.name.trim(), category: draft.category.trim(), description: draft.description.trim(), criteria: draft.criteria.trim(), severityCriteria: serializeSeverityLevels(levels), severityLevels: levels, care: draft.care.trim(), recommendation: draft.recommendation.trim() })
  }
